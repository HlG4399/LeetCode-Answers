...
public:
    vector<ListNode*> splitListToParts(ListNode* root, int k) {
        vector<ListNode*> results(k,nullptr);
        int len=0;
        ListNode* temp_root;
        for(temp_root=root;temp_root;temp_root=temp_root->next)
        {
            len++;
        }
        int per_num=len/k,remainder=len%k;
        for(int i=0;i!=k;i++,remainder--)
        {
            results[i]=root;
            for(int j=0;j<per_num+(remainder>0);j++)
            {
                temp_root=root;
                root=root->next;
            }
